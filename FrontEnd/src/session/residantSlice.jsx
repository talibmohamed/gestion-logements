...
const residantSlice = createSlice({
  ...
  reducers: {
    ...
    removeResidant: (state, action) => {
      state.residants = state.residants.filter(
        (residant) => residant.res_id !== action.payload.res_id
      );
    },
    updateResidant: (state, action) => {
      const index = state.residants.findIndex(
        (residant) => residant.res_id === action.payload.res_id
      );
      if (index !== -1) {
...
export const { setResidants, addResidant, removeResidant, updateResidant } =
  residantSlice.actions;
